...
- **名称**（必填）— 例如 "我的交易机器人"
- **描述**（可选）— 这个想法是关于什么的
- **项目路径**（可选）— 点击文件夹按钮浏览并选择本地目录（也可以新建文件夹）。Claude Code 将在这个目录下运行。

> **提示：** 如果设置了项目路径，EZVibe 会自动从 `~/.claude/` 读取该项目的 Claude Code 记忆和 CLAUDE.md。
...
| 标签页 | 显示内容 | 数据来源 |
|---|---|---|
| **Instructions** | 编辑项目的 `CLAUDE.md` — Claude Code 启动时读取的项目指令 | `{项目目录}/CLAUDE.md` |
| **记忆** | 项目相关的记忆文件 | `~/.claude/projects/{key}/memory/` |
| **笔记** | 你的自由笔记（输入时自动保存） | SQLite 数据库 |
| **Token** | 全局 token 用量统计、模型分布、每日活跃度 | `~/.claude/stats-cache.json` |

这意味着你可以在与 Claude Code 交互的*同时*编辑项目指令或查看 token 用量 — 不再需要切换窗口。

### 第四步：管理多个 Idea

- **切换：** 点击侧边栏中任意 idea — 终端即时切换，缓冲区自动恢复
- **后台运行：** 切换走后终端会话继续在后台运行
- **编辑：** 点击工具栏的 "Edit" 修改名称、描述、项目路径或阶段
- **移除：** 点击 "Remove" 从 EZVibe 中移除 idea（磁盘上的项目文件不会被删除）
