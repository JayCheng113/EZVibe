      ...
      {plans.map(plan => (
        <div key={plan.filename}>
          <button
            ...
            className="flex w-full items-center gap-2 rounded px-3 py-2 text-left text-sm hover:bg-gray-800"
          >
            <span className="text-gray-500">{expanded === plan.filename ? '▾' : '▸'}</span>
            <span className="truncate text-gray-200">{plan.title}</span>
            <span className="ml-auto text-xs text-gray-600">{plan.filename}</span>
          </button>
          {expanded === plan.filename && (
            <pre className="mx-3 mb-2 max-h-80 overflow-auto rounded bg-gray-900/50 p-3 text-xs text-gray-300">
              {plan.content}
            </pre>
          )}
        </div>
